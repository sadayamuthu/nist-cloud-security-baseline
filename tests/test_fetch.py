...
from ncsb.fetch import NCSB_CATALOG_URL, main
...
def test_fetch_writes_catalog_to_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("ncsb.fetch.requests.get", return_value=mock_resp) as mock_get:
            main(args)

        mock_get.assert_called_once_with(NCSB_CATALOG_URL, timeout=30)
...
def test_fetch_prints_fetching_url(capsys):
    ...
    captured = capsys.readouterr()
    assert NCSB_CATALOG_URL in captured.out
